...
Cocktail Private Hub를 설치하기 위한 VM또는 machine에서 먼저 Docker를 설치한다. 아래는 해당  장비의 ip를  192.168.50.13으로 가정한다.
...
참고 사이트 \([https://github.com/vmware/harbor/blob/master/docs/configure\_https.md\](https://github.com/vmware/harbor/blob/master/docs/configure_https.md%29\)
...
# ls -al
-rw-r--r--. 1 root root 1976 Dec  7 11:05 ca.crt
-rw-r--r--. 1 root root 3272 Dec  7 11:05 ca.key
-rw-r--r--. 1 root root   17 Dec  7 11:07 ca.srl
-rw-r--r--. 1 root root   34 Dec  7 11:06 extfile.cnf
-rw-r--r--. 1 root root 1895 Dec  7 11:07 harbor.crt
-rw-r--r--. 1 root root 1679 Dec  7 11:05 harbor.csr
-rw-r--r--. 1 root root 3272 Dec  7 11:05 harbor.key
```
...
harbor_admin_password = C0ckt@1!Admin         (harbor admin user의 password. 이 값은 cocktail api server에서 사용)
...
```
...
```
[root@cph harbor]# docker login 192.168.50.13
Username: admin
Password:
Error response from daemon: Get https://192.168.50.13/v2/: x509: certificate signed by unknown authority
```
...
이제 사용자가 원격 harbor에 Self Signed Certificate로 harbor에 접속하기 위해 harbor의 ca.csr 파일을 아래 위치에 저장한다.

```
mkdir -p /etc/docker/certs.d/192.168.50.13
이후 ca.crt를 복사한다.
```
...
```
upload create-docker-tls.sh

# cd cocktail
# mkdir buid; cd build
# ./create-docker-tls.sh 192.168.50.13
# cd /root/.docker
# ls -al
...
--tlsverify \
--tlscacert=/root/.docker/ca.pem --tlscert=/root/.docker/server-cert.pem \
